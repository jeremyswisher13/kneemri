      ...
      cards: [
        {
          ...
          answer: "In the dependent recesses — the coronoid fossa, the olecranon fossa, and the radiocapitellar recess — best seen against bright joint fluid on sagittal fluid-sensitive images (GRE blooms ossified bodies; CT/CT arthrography is most sensitive for small ossified bodies and for counting them pre-op). Exclude a thickened radiocapitellar plica / posterolateral synovial fold, which can cause lateral catching and mimics a loose body or OCD. Thickness helps but does not decide: the radiohumeral plica averages about 1.8 mm in asymptomatic elbows versus 2.5 mm in synovial fringe syndrome, a difference that is NOT statistically significant on mean thickness — only the proportion exceeding roughly 2.6 mm separates the groups (67% vs 13%). A separate 3 mm figure comes from the POSTERIOR (olecranon-level) fold, and that study likewise reported substantial overlap between symptomatic and asymptomatic plicae. Reconcile that with the posterolateral radiocapitellar fold, which is present in about three-quarters of ASYMPTOMATIC elbows and averages about 3 mm there, with symptomatic series running nearer 7 mm mediolateral: about 3 mm is a NORMAL value, not a pathologic threshold, and no cutoff is validated. So no number makes the diagnosis — call the plica clinically relevant only when it is clearly enlarged relative to normal (or covers a substantial portion of the radial head) and/or edematous AND the mechanical symptoms are concordant, never as a stand-alone measurement. And do not ascribe every loose body to a throwing lesion: in the non-thrower/laborer, primary elbow OA is the most common source.",
        },
      ],
    },
  ],
